...
export async function playAuthIntro({force=false}={}){
  ...
  if(!force&&auth.dataset.ffMotionV4==='done')return;
  auth.dataset.ffMotionV4='done';
...
export function restartAuthIntro(){
  const auth=byId('auth-view');
  if(auth)auth.dataset.ffMotionV4='';
...
export function animateDashboardMoney(targets,duration=1650){
...
export function revealDashboard({moneyTargets,onCovered,onComplete}={}){
  ...
  window.setTimeout(()=>{
    ...
    window.setTimeout(()=>animateDashboardMoney(moneyTargets||[],1650),90);
