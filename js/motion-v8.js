...
export async function playAuthIntro({force=false}={}){
  ...
  if(!force&&auth.dataset.ffMotionV8==='done')return;
  auth.dataset.ffMotionV8='done';
...
export function restartAuthIntro(){
  const auth=byId('auth-view');
  if(auth)auth.dataset.ffMotionV8='';
...
export function animateDashboardMoney(targets,duration=5000){
  if(!targets?.length)return;
  const start=performance.now();
  const frame=now=>{
    const t=Math.min(1,(now-start)/duration);
    // Progressão linear: os 5 segundos são percebidos de forma uniforme.
    const eased=t;
...
export function revealDashboard({moneyTargets,onCovered,onComplete}={}){
  ...
  window.setTimeout(()=>{
    ...
    window.setTimeout(()=>animateDashboardMoney(moneyTargets||[],5000),90);
