...
export async function playAuthIntro({force=false}={}){
  ...
  if(!force&&auth.dataset.ffMotionV10==='done')return;
  auth.dataset.ffMotionV10='done';
...
export function restartAuthIntro(){
  const auth=byId('auth-view');
  if(auth)auth.dataset.ffMotionV10='';
...
export function animateDashboardMoney(targets,duration=1350){
  if(!targets?.length)return;
  const start=performance.now();
  const frame=now=>{
    const t=Math.min(1,(now-start)/duration);
    // Progressão linear. O count-up ocupa os 1,35 s finais do ciclo total de 2 s.
    const eased=t;
...
export function revealDashboard({moneyTargets,onCovered,onComplete}={}){
  ...
  window.setTimeout(()=>onCovered?.(),320);
  window.setTimeout(()=>{
    ...
    animateDashboardMoney(moneyTargets||[],1350);
    window.setTimeout(()=>app?.classList.remove('ff-motion-dashboard-in'),500);
    onComplete?.();
  },650);
}
