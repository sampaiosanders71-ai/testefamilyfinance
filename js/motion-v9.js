...
export async function playAuthIntro({force=false}={}){
  ...
  if(!force&&auth.dataset.ffMotionV9==='done')return;
  auth.dataset.ffMotionV9='done';
...
export function restartAuthIntro(){
  const auth=byId('auth-view');
  if(auth)auth.dataset.ffMotionV9='';
...
export function animateDashboardMoney(targets,duration=3000){
...
export function revealDashboard({moneyTargets,onCovered,onComplete}={}){
  ...
  window.setTimeout(()=>{
    ...
    window.setTimeout(()=>animateDashboardMoney(moneyTargets||[],3000),90);
