...
async function fetchTransactionSnapshot(userId) {
  const firstTo = TRANSACTION_PAGE_SIZE - 1;
  ...
  const expected = Number(first.count || 0);
  const rows = [...(first.data || [])];
  for (let from = TRANSACTION_PAGE_SIZE; from < expected; from += TRANSACTION_PAGE_SIZE) {
    const to = Math.min(from + TRANSACTION_PAGE_SIZE - 1, expected - 1);
...
export async function listTransactions() {
  ...
  for (let attempt = 1; attempt <= TRANSACTION_SYNC_ATTEMPTS; attempt += 1) {
    // A primeira página já retorna a contagem exata, evitando uma chamada
    // extra antes da leitura. A contagem final continua garantindo integridade.
    const snapshot = await fetchTransactionSnapshot(user.id);
    const countAfter = await countTransactions(user.id);
    const uniqueRows = [...new Map(snapshot.rows.map(row => [row.id, row])).values()];

    lastExpected = countAfter;
    lastLoaded = uniqueRows.length;

    if (snapshot.expected === countAfter && uniqueRows.length === countAfter && snapshot.rows.length === uniqueRows.length) {
