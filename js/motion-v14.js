...
function createSplash() {
  byId('ff-v14-splash')?.remove();
  const splash = document.createElement('div');
  splash.id = 'ff-v14-splash';
  splash.className = 'ff-v14-splash';
  splash.setAttribute('aria-hidden', 'true');

  const brand = document.createElement('div');
  brand.className = 'ff-v14-splash-brand';
...
export async function playAuthIntro({ force = false } = {}) {
  ...
  auth.classList.add('ff-v14-intro');
  auth.classList.remove('ff-v14-panel-in', 'ff-v14-fields-in');

  const { splash, brand } = createSplash();
  await animateBrand(brand);
  if (generation !== introGeneration) return;

  auth.classList.add('ff-v14-panel-in');
  await sleep(520);
  auth.classList.add('ff-v14-fields-in');
  await sleep(780);
...
export function setLoginButtonMotion(busy, mode = 'login', phase = 'auth') {
  ...
  document.documentElement.classList.toggle('ff-v14-login-busy', Boolean(busy));
  btn.classList.toggle('ff-v14-login-capsule', Boolean(busy));

  if (busy) {
    btn.innerHTML = '<span class="ff-v14-login-spinner" aria-hidden="true"></span>';
...
function ensureDashboardWipe() {
  let wipe = byId('ff-v14-dashboard-wipe');
  if (!wipe) {
    wipe = document.createElement('div');
    wipe.id = 'ff-v14-dashboard-wipe';
    wipe.className = 'ff-v14-dashboard-wipe';
...
export function settleDashboardMoneyChange() {
  ...
  for (const target of active.targets) {
    ...
    target.el.classList.remove('ff-v14-value-changing');
  }
}
...
export function prepareDashboardMoneyChange(previousState) {
  ...
  for (const target of targets) {
    target.el.classList.add('ff-v14-value-changing');
...
function resetDashboardMoney(targets) {
  for (const { el } of targets) {
    el.classList.add('ff-v14-counting');
...
function animateDashboardMoney(targets, duration = DASHBOARD_COUNT_MS) {
  ...
  const frame = now => {
    ...
    for (const { el, value } of targets) {
      ...
      el.classList.remove('ff-v14-counting');
    }
  };
...
export function revealDashboard({ moneyTargets = [], onCovered, onComplete } = {}) {
  ...
  app?.classList.remove('ff-v14-dashboard-in');
  document.documentElement.classList.remove('ff-v14-dashboard-transition');
  resetDashboardMoney(moneyTargets);

  // t = 0 ms
  document.documentElement.classList.add('ff-v14-dashboard-transition');
  ...
  window.setTimeout(() => {
    app?.classList.add('ff-v14-dashboard-in');
  ...
  window.setTimeout(() => app?.classList.remove('ff-v14-dashboard-in'), 900);
