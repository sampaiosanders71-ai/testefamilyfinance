import { prepareAuthMotion, resetAuthMotion, setAuthMotionBusy, revealAppWithMotion } from './auth-motion.js';
...
export function showAuth(){
  ...
  setAuthMotionBusy(false);
  void resetAuthMotion();
}
export function showApp(user,profile){
  ...
  /* O Dashboard já está pronto por baixo do wipe; o login só some quando a tela fica coberta. */
  app.classList.remove('hidden');
  mobile?.classList.add('hidden');
  revealAppWithMotion({
    onCover:()=>auth.classList.add('hidden'),
    onComplete:()=>{
...
export function setAuthBusy(busy,mode,phase='auth'){
  ...
  if(btn){
    btn.disabled=busy;
    btn.innerHTML=busy?'<span class="ff-auth-button-spinner" aria-hidden="true"></span>':(register?'Criar conta':'Entrar');
    btn.setAttribute('aria-label',busy?(phase==='data'?'Carregando seus dados':(register?'Criando conta':'Entrando')):(register?'Criar conta':'Entrar'));
  }
