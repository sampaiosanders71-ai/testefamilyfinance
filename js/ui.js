...
export function setAuthMode(mode){const register=mode==='register';document.getElementById('name-field').classList.toggle('hidden',!register);document.getElementById('auth-title').textContent=register?'Criar conta':'Entrar';document.getElementById('auth-subtitle').textContent=register?'Crie sua conta nova do Family Finance.':'Entre na reconstrução com Supabase Auth.';document.getElementById('auth-submit').textContent=register?'Criar conta':'Entrar';document.getElementById('auth-toggle').textContent=register?'Já tenho uma conta':'Criar uma conta nova';document.getElementById('password').autocomplete=register?'new-password':'current-password'}
...
export function showApp(user,profile){document.getElementById('auth-view').classList.add('hidden');document.getElementById('app-view').classList.remove('hidden');document.getElementById('mobile-nav')?.classList.remove('hidden');const name=profile?.display_name||user.user_metadata?.display_name||user.email?.split('@')[0]||'Usuário';document.getElementById('user-name').textContent=name;document.getElementById('user-email').textContent=user.email||'';document.getElementById('user-avatar').textContent=name.trim().charAt(0).toUpperCase()||'U';document.getElementById('home-greeting').textContent=`Olá, ${name}.`;applyTheme(profile?.theme||'system')}
